...
# # Draw centroids on the lanes (all green for centroids inside lanes)
...
# 10% distance threshold for assigning centroids to lanes
threshold_distance = 0.01 * min(img_width, img_height)
...
# Reassign centroids that are close to lanes (blue) or leave them outside (red)
...
for centroid in centroids_outside_lane:
    ...
    # Find the minimum distance to any contour
    for contour in contours:
        distance = cv2.pointPolygonTest(contour, centroid, True)  # Get distance to contour
...
plt.title('Lane Detection with Centroid Assignment and Distance-based Reassignment')
plt.axis('off')

...
for line in lines:
    ...
    cv2.putText(test_image,str(lane_id), (midpoint_x,midpoint_y), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), 1)
    line = line.reshape((-1, 1, 2)).astype(np.int32)
    cv2.polylines(test_image, [line], isClosed=False, color=(0, 255, 0), thickness=1)
    lane_id+=1
...
for centroid in centroids:
    ...
    cv2.circle(test_image, coord, 2, (0, 200, 0), -1)
    # This prints out the coordinates of the centroids on the image
    cv2.putText(test_image,"(" + str(centroid[0]) +", " + str(centroid[1]) +")", (centroid[0] + 4, centroid[1] + 4), cv2.FONT_HERSHEY_SIMPLEX, 0.2, (255, 0, 0), 1)
    centroid_line_assignment, projection, isValid = line_assignment_by_perpendicular_distance(coord, lines)
    if isValid is True:
        cv2.circle(test_image, (int(projection[0]), int(projection[1])), 2, color_dict.get(centroid[2]), -1)
        ...
        cv2.circle(test_image, coord, 2, (0,0,0), -1)
...
#     for centroid in centroids:
#         coord = (centroid[0], centroid[1])
#         cv2.11(test_image, coord, 5, (0, 200, 0), -1)
#         # This prints out the coordinates of the centroids on the image

...
#get density
def get_density_score(camid, date):

    return 5


#get count
def get_queue_length(camid, laneid, date):

    return 5


def get_overall_score(date):
    lane_groupings = {}
    total_counts = []
    ...
    for img_idx, img in enumerate(lane_groupings):
        for group_idx, group in enumerate(img):
            density_score = get_density_score(img, date)
            total_count = 0
            for lane in group:
                queue_length = get_queue_length(img, lane, date)  
